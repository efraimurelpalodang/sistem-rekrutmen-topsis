...
class CekSudahTesMiddleware
{
   ...
   *
   * Memastikan pelamar hanya bisa akses halaman tes jika:
   * 1. Lamaran tersebut miliknya
   * 2. Status lamaran adalah 'menunggu_tes'
   * 3. Belum pernah submit hasil tes sebelumnya
   */
  public function handle(Request $request, Closure $next): Response
  {
    $lamaranId = $request->route('lamaranId');
    $lamaran = Lamaran::with('hasilTes')->findOrFail($lamaranId);
